...
public class Day12Deel2 {
	...
	public static void main(String[] args) {
		...
		String[][] pipes = new String[1000][1000];
		pipes[0][0] = "0";
		...
		for (int loop = 0; loop <= 100; loop++) {
			...
			for (int i = 0; i < programmas.length; i++) {
				...
				for (int j = 0; j < perPipe.length; j++) {
					int nieuwPipeNodig = 0;
					if (perPipe[j] != null) {
						for (int nieuwePipe = 0; nieuwePipe < pipes.length; nieuwePipe++) {
							
							for (int k = 0; k < pipes[nieuwePipe].length; k++) {
								if (pipes[nieuwePipe][k] != null) {
									if (temp1[0].equals(pipes[nieuwePipe][k])) {
										nieuwPipeNodig++;
										for (int aantalP = 0; aantalP < perPipe.length; aantalP++) {
											int counter = 0;
											for (int o = 0; o < pipes[nieuwePipe].length; o++) {
												if (pipes[nieuwePipe][o] != null) {
													if (pipes[nieuwePipe][o].equals(perPipe[aantalP])) {
														counter++;
													}
												}
											}
											if (counter == 0) {
												pipes[nieuwePipe][count] = perPipe[aantalP];
												System.out
														.println("Nieuwe unieke pipe naar " + pipes[nieuwePipe][0] + "! nl.: " + perPipe[aantalP]);
												count++;
											}
										}
									}
